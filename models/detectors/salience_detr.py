...
class SalienceDETR(DNDETRDetector):
    ...
    def forward(self, images: List[Tensor], targets: List[Dict] = None):
        ...
        if self.msrcr_enhanced is not None:
            ...
            # 反归一化：将归一化后的图像恢复到 [0, 1] 范围
            denormalized = images.tensors * std + mean
            # 确保值在 [0, 1] 范围内
            denormalized = torch.clamp(denormalized, 0.0, 1.0)
            
            # 应用 MSRCR 增强
            enhanced = self.msrcr_enhanced(denormalized)
            
            # 重新归一化
            enhanced = torch.clamp(enhanced, 0.0, 1.0)  # 确保在 [0, 1] 范围内
            enhanced_normalized = (enhanced - mean) / std
            
            # 更新 ImageList 中的张量
            images.tensors = enhanced_normalized
