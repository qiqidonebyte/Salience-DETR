...
# ========== 轻量级参数配置（针对CPU训练优化）==========
# 原始配置参考: configs/salience_detr/salience_detr_resnet50_800_1333.py
embed_dim = 256  # 保持256，降低会影响精度 | 原始: 256
num_classes = 91  # 原始: 91
num_queries = 300  # CPU优化: 300 | 原始: 900
num_feature_levels = 4  # 原始: 4
transformer_enc_layers = 4  # CPU优化: 4 | 原始: 6
transformer_dec_layers = 4  # CPU优化: 4 | 原始: 6
num_heads = 8  # 原始: 8
dim_feedforward = 1024  # CPU优化: 1024 | 原始: 2048
...
transformer = SalienceTransformer(
    ...
    level_filter_ratio=(0.4, 0.8, 1.0, 1.0),  # 原始: (0.4, 0.8, 1.0, 1.0)
    layer_filter_ratio=(1.0, 0.8, 0.6, 0.4),  # CPU优化: 4层 | 原始: (1.0, 0.8, 0.6, 0.6, 0.4, 0.2) 6层
)
...
model = PhysAwareDETR(
    ...
    aux_loss=True,
    min_size=480,  # CPU优化: 480 | 原始: 800
    max_size=640,  # CPU优化: 640 | 原始: 1333
    msrcr_enhanced=msrcr_enhanced,  # CPU优化: None | 原始: MSRCREnhanced实例
)
