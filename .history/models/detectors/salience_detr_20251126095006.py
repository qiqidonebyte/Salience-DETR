...
class PhysAwareDETR(DNDETRDetector):
    def __init__(
        ...
        if self.msrcr_enhanced is not None:
            # 初始化为较小的值，避免过度放大特征，影响训练稳定性
            # 使用0.1作为初始值，让模型逐步学习如何利用物理注意力
    ...
    def forward(self, images: List[Tensor], targets: List[Dict] = None):
        ...
        if self.msrcr_enhanced is not None:
            ...
            attention_map = self.msrcr_enhanced(denormalized)
    ...
    def _apply_physical_attention(self, features: List[Tensor], attention_map: Tensor) -> List[Tensor]:
        ...
        for level_idx, feat in enumerate(features):
            ...
            # 使用更稳定的融合方式：残差连接 + 门控机制
            # 限制 level_strength 的影响范围，避免过度放大特征
            # 使用 tanh 来限制强度范围，使其在 [-1, 1] 之间，然后缩放到 [0, 2]
            gated_attention = torch.tanh(level_strength) * attn_resized
            fused_features.append(feat * (1.0 + gated_attention))
