...
# 1. 物理光学增强模块 (MSRCR简化版) - 性能优化版本
class MSRCREnhanced(nn.Module):
    def __init__(self, scales=[15, 80, 250], weights=[1.0, 1.0, 1.0], use_separable=True, downsample_threshold=100):
        """
        Args:
            scales: 多尺度高斯核的尺度参数
            weights: 各尺度的权重
            use_separable: 是否使用可分离卷积（1D卷积两次，大幅提升速度）
            downsample_threshold: 当sigma大于此值时，使用下采样加速
        """
        super().__init__()
        self.scales = scales
