...
# ========== CPU训练优化配置（轻量级）==========
num_epochs = 5  # train epochs (CPU训练优化: 5) | 原始配置: 48
batch_size = 1  # total_batch_size = #GPU x batch_size
num_workers = 0  # workers for pytorch DataLoader (set to 0 on Windows to avoid pickle issues with lambda functions) | 原始配置: 4 (服务器上可改回4)
pin_memory = True  # whether pin_memory for pytorch DataLoader
print_freq = 10  # frequency to print logs (CPU训练优化: 10) | 原始配置: 50
starting_epoch = 0
...
coco_path = '../seadronessee/Downloads/Uncompressed Version'  # 去掉末尾斜杠，避免路径拼接时出现双斜杠
...
# 0715 当前最佳
# ========== CPU训练优化配置 ==========
resume_from_checkpoint = None  # CPU训练建议从头开始
# ========== 原始配置（服务器训练可恢复checkpoint）==========
